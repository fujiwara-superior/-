...
unit_rows = [
    ...
    ("専門家による監修費（1科目あたり）", 350000,
     "労働安全衛生コンサルタント等の第三者監修。法定の科目・範囲・時間を満たしていることを、監修者名を明示して確認します。"),
...
for t in [
    ...
    "5. 第三者の専門家が監修し、法定の科目・範囲・時間を満たしていることを監修者名を明示して確認します。",
    "",
    "■ 2科目目以降について",
    "・章立てのテンプレート、スライドのデザイン、動画の書き出し設定は1科目目で確立するため、2科目目以降は制作単価を抑えられます。",
    "・目安として、2科目目以降は上表の8割程度を想定してください（監修費は科目ごとに必要です）。",
]:
...
BLOCKS = [
    ...
    ("■ 2. 教材内容の監修について", NAVY, [
        ("・教材の法令適合性を確認できる専門家（労働安全衛生コンサルタント等）は、弊社が手配し、費用に含めてご提示します。", None),
        ("・聖建様に、監修内容をご自身で調査・判断していただく必要はありません。", None),
        ("・ただし、監修は無償では行えません。1科目あたりの監修費として、お見積りに計上させていただきます。", None),
        ("・監修者の氏名・資格は教材内および修了証に明示します。これにより、第三者から根拠を問われた際に示せる状態になります。", None),
        ("・弁護士による確認では代替できません。労働安全衛生教育の内容の妥当性は、法律実務ではなく安全衛生の専門領域だからです。", "B3402F"),
    ]),
    ("■ 3. 著作権に関する前提", NAVY, [
