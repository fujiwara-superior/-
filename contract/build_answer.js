  ...
  rows: [new TableRow({ children: [
    new TableCell({
      ...
      children: [
        ...
        P(T('営業部　津隈 佑己 様', { size: 20, color: INK2 }), { after: 0 }),
      ],
    }),
    new TableCell({
      width: { size: W - 4600, type: WidthType.DXA },
      margins: { top: 0, bottom: 0, left: 0, right: 0 },
      children: [
        ...
        P(T('〒464-0075　名古屋市千種区内山3-18-10', { size: 18, color: INK2 }), { align: AlignmentType.RIGHT, after: 20 }),
...
body.push(NOTE('プログラムの著作権は、制作した者に発生します', [
  ['請負契約で制作した場合であっても、',
   T('契約に定めがなければ著作権は受託者に留まります', { bold: true }),
   '（著作権法第17条、第61条）。「いつ移転するか」は、契約でどう定めるかの問題となります。そのうえで、弊社からは次の切り分けをご提案いたします。'],
], 'F1F5F5', NAVY));
...
body.push(NOTE('GitHub（ギットハブ）の閲覧権限について', [
  ...
  ['「何を作るか」を1件ずつ登録した作業一覧と、それを「未着手／作業中／確認待ち／完了」の列に並べたボードです。付箋を貼り替えていくホワイトボードのような画面とお考えください。担当者と期限も表示されます。'],
...
body.push(NOTE('「スペリオルに何かあったらどうするのか」へのお答え', [
  ['ソースコードを', T('第三者機関に預託（エスクロー）', { bold: true }),
   'いたします。一般財団法人ソフトウェア情報センター等の中立的な機関を想定しています。'],
...
body.push(P('これらの制御は、法定の学科時間を実質的に満たしたことを記録で示すための中核機能です。詳細は本人確認システム設計書をご参照ください。', { before: 80 }));
...
body.push(P('貴社発行の契約書に弊社が署名する形で差し支えございません。'));
body.push(NOTE('署名の前に、契約書案を拝見させてください', [
  ['今回のお見積りは、', T('ご発注時に全額を一括でお支払いいただくことを前提に、貴社特別値引き 350,000円を計上しております。', { bold: true }),
...
body.push(LI('事前に作業範囲を貴社へご報告し、ご承諾をいただいたうえで行います'));
body.push(LI('協力会社に対し、弊社と同等の秘密保持義務を課します'));
body.push(LI([T('成果物の品質および履行の責任は、すべて弊社が負います。', { bold: true }), '貴社の窓口は弊社に一本化されます']));
body.push(P('契約書に再委託の禁止条項を設けられる場合は、上記のような「事前の書面による承諾を得た場合はこの限りでない」旨を併記いただけますと幸いです。', { before: 80 }));
